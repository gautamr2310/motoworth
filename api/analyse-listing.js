...
const catalogue = require('../data/catalogue.v2.json');
...
function getMeta(html, key, attr = 'property') {
  ...
  const re = new RegExp(`<meta[^>]+${attr}=["']${safeKey}["'][^>]+content=["']([^"']+)["'][^>]*>`, 'i');
  const re2 = new RegExp(`<meta[^>]+content=["']([^"']+)["'][^>]+${attr}=["']${safeKey}["'][^>]*>`, 'i');
...
function extractPrice(text) {
  ...
  const freq = new Map();
  for (const value of candidates) freq.set(value, (freq.get(value) || 0) + 1);
  return [...freq.entries()].sort((a, b) => b[1] - a[1] || a[0] - b[0])[0][0];
}

function extractKm(text) {
  const re = /(\d{1,3}(?:,\d{3})*|\d{4,7})\s*(?:km|kms|kilometers|kilometres)\b/ig;
  const vals = [];
  let m;
  while ((m = re.exec(text))) {
  ...
  return vals.length ? vals.sort((a, b) => a - b)[0] : null;
}
...
function extractYear(text) {
  ...
  const labeled = [...text.matchAll(/(?:year|model|registered|registration|regn|reg\.?)\D{0,20}(20\d{2})/ig)]
    .map(m => Number(m[1]))
    .filter(y => y >= 2000 && y <= new Date().getFullYear());
...
async function ensurePublicHost(url) {
  const addresses = (await dns.lookup(url.hostname, { all: true })).map(x => x.address);
  if (!addresses.length || addresses.some(isPrivateIp)) throw new Error('private-host');
}

function matchModel(title, description) {
  ...
  const candidates = catalogue.models.map(m => {
    ...
    return { m, score, brand: brand?.name || '' };
  ...
  return { id: best.m.id, brand: best.brand, model: best.m.name, confidence };
}
...
function pickJsonLd(jsonlds) {
  ...
  for (const j of jsonlds) {
    ...
    if (!out.year) out.year = Number(j.vehicleModelDate || j.modelDate || j.productionDate) || null;
    if (!out.km) {
    ...
    if (!out.sellerType) {
      const seller = j.seller || offer?.seller;
...
async function handler(req, res) {
  if (req.method !== 'POST') return bad(res, 405, 'Use POST for listing analysis.');

  let body = req.body;
  if (typeof body === 'string') {
    try { body = JSON.parse(body); } catch (_) { return bad(res, 400, 'Send JSON with a listingUrl.'); }
  }

  const raw = String(body?.listingUrl || '').trim();
  if (!raw) return bad(res, 400, 'Listing URL is required.');

  let url;
  try { url = new URL(raw); } catch (_) { return bad(res, 400, 'That does not look like a valid URL.'); }
  if (!['http:', 'https:'].includes(url.protocol)) return bad(res, 400, 'Only http and https listing URLs are supported.');
  if (url.port && !['80', '443'].includes(url.port)) return bad(res, 400, 'Custom ports are not supported.');

  try { await ensurePublicHost(url); } catch (_) { return bad(res, 400, 'That listing URL cannot be fetched safely.'); }

  let fetchResponse;
  try {
    fetchResponse = await fetch(canonicalUrl(raw), {
      redirect: 'follow',
      signal: AbortSignal.timeout(12000),
      headers: {
        'user-agent': 'Mozilla/5.0 (compatible; MotoWorthBot/1.0; +https://www.motoworth.com)',
        'accept': 'text/html,application/xhtml+xml,application/xml;q=0.9,*/*;q=0.8'
      }
    });
  } catch (_) {
    return bad(res, 502, 'MotoWorth could not fetch this listing. The marketplace may require a browser session or block automated requests.', { fallback: 'manual' });
  }

  try {
    const finalUrl = new URL(fetchResponse.url);
    await ensurePublicHost(finalUrl);
  } catch (_) {
    return bad(res, 502, 'The listing redirected to a URL MotoWorth cannot safely fetch.', { fallback: 'manual' });
  }

  if (!fetchResponse.ok) return bad(res, 502, `The listing returned HTTP ${fetchResponse.status}.`, { fallback: 'manual' });
  const contentType = fetchResponse.headers.get('content-type') || '';
  if (!contentType.includes('text/html') && !contentType.includes('application/xhtml+xml')) return bad(res, 415, 'That URL does not return an HTML listing page.', { fallback: 'manual' });

  const html = (await fetchResponse.text()).slice(0, 3_500_000);
  ...
  const price = ld.price || extractPrice(combinedText);
  const km = ld.km || extractKm(combinedText);
  const year = ld.year || extractYear(combinedText);
  const city = extractCity(`${ld.city} ${combinedText}`) || null;
  const seller = ld.sellerType ? { value: ld.sellerType, confidence: 'high' } : inferSeller(combinedText);
  const condition = inferCondition(`${title} ${description}`);

  const model = matchModel(title, `${description} ${bodyText.slice(0, 10000)}`);
  const matchedModel = catalogue.models.find(m => m.id === model?.id);
  const listingText = normalize(`${title} ${description}`);
  const variant = matchedModel ? catalogue.variants.find(v => v.model_id === matchedModel.id && listingText.includes(normalize(v.name))) : null;
  ...
  return res.status(200).json({
    ok: true,
    source: { url: canonicalUrl(raw), final_url: fetchResponse.url, domain: url.hostname, fetched_at: new Date().toISOString() },
    listing: {
      title: decodeHtml(title).slice(0, 220),
      description: decodeHtml(description).slice(0, 1200),
      image,
      fields,
      matched_model_id: model?.id || null,
      extraction_confidence: confidence,
      missing_fields: missing
    },
    notes: [
      'Fields were extracted from publicly returned page metadata/HTML and should be confirmed before purchase.',
